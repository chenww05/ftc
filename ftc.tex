...
\newcommand{\rev}[1]{\color{black} #1\color{blue}}
...
Equation~\ref{eq:theta-1} means the posterior estimation of $\theta_{\gamma}$ is first controlled by the prior knowledge ($a_{\gamma}$ and $b_{\gamma}$) and then gradually adjusted by observed data ($n$ and $x_i$). The MLE (Maximum Likelihood Estimation) of the scale parameter $\theta_{\gamma}$ is defined as:
...
It is difficult to find an analytical solution of $k^*$. However, there are a few constraints that can 
