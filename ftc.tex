...
process, only the prior knowledge \rev{(shape and scale parameters of a Inverse-Gamma distribution for the inter-arrival failures, and a Gamma distribution for the task runtime and system overhead) } is used to estimate the MLEs of the unknown parameters \rev{(scales parameters of the inter-arrival failures $\theta_\gamma$, the task runtime $\theta_t$, and the system overhead $\theta_s$)}. In contrast,
