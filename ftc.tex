...
been addressed. For instance, the Pegasus WMS~\cite{Deelman2004} has incorporated 
...
fits several WMS such as Pegasus~\cite{Deelman2004}, Askalon~\cite{fahringer2007askalon}, and 
...
workflow~\cite{Deelman2004} provided by the user or a workflow composition system. It restructures 
...
%The static estimation process only utilizes the prior knowledge and the dynamic estimation process uses both the prior and the posterior knowledge. 
...
in~\cite{nadarajah2008review}. For the sake of simplicity, we limit this work to show a typical case 
where the system overhead and the task runtime have the same scale parameter 
...
Fig.~\ref{fig:model-size}) when $\theta_{\gamma} \in \{20,30,40\}$ is 2, 3, and 5 respectively. 
\todo[inline]{there are no red dots in Fig~\ref{fig:model-size}.}
...
\begin{figure}[!htb]
\centering
  \includegraphics[width=0.65\linewidth]{hcr.pdf}
...
\begin{figure}[!htb]
\centering
  \includegraphics[width=0.58\linewidth]{sr.pdf}
...
\begin{figure}[!htb]
\centering
  \includegraphics[width=0.60\linewidth]{dr.pdf}
...
\begin{figure}[!htb]
\centering
  \includegraphics[width=0.95\linewidth]{vr.pdf}
...
%The mapping software can do one of two major tasks.
...
We collected workflow execution traces~\cite{Juve2013, Chen2011} (including overhead and task runtime information) from real runs (executed on FutureGrid and Amazon EC2) of the scientific workflow applications described in Section~\ref{sec:applications}. The traces are used to feed the Workflow Generator toolkit~\cite{rafael2014community} to generate synthetic workflows. This allows us to perform simulations with several different configurations under controlled conditions. The toolkit uses the information gathered from actual scientific workflow executions to generate synthetic workflows resembling those used by real world scientific applications. The number of inputs to be processed, the number of tasks in the workflow, and their composition determine the structure of the generated workflow. Such an approach of traced based simulation allows us to utilize real traces and vary the system setting (i.e., the inter-arrival time of failures) and workflow (i.e., avg. task runtime) to fully explore the performance of our fault-tolerant clustering algorithms. 
