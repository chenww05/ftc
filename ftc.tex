...
performance of \rev{workflow executions } in faulty execution environments. Experimental results show that failures can have significant 
...
be recomputed, thereby resource cycles are wasted. In addition, there is no guarantee that the 
task will recompute successfully.  As an alternative, jobs can be replicated to avoid failures 
related to one specific worker node~\cite{Plankensteiner2009}. However, task replication
...
\rev{There have been a considerable amount of work trying to solve workflow-mapping problem using DAG scheduling heuristics such as HEFT \cite{Topcuoglu2002}, Min-Min \cite{Blythe2005}, MaxMin \cite{Braun2001}, MCT \cite{Braun2001}, etc. Duan \cite{Rubing2005} and Wieczorek \cite{Wieczorek2005} have discussed the scheduling of scientific workflows in dynamic grids. Condor \cite{Kalayci2010} has used matchmaking to avoid scheduling tasks to compute nodes without sufficient resources (CPU power, etc.). In our previous work~\cite{Chen2012}, we adopted a similar approach to avoid schedule workflow tasks to compute nodes with a high failure rate. In this work, we focus on the performance gain of task clustering and in particular, how to adjust the clustering size to balance the cost of task retry and the scheduling overheads.}
...
workflow~\cite{Deelman2005, Deelman-FGCS-2014} provided by the user or a workflow 
...
workflow mapper is also used to merge fine-grained tasks into a single clustered job 
(i.e., \emph{task clustering}). A job then is a single execution unit in the workflow execution 
systems and is composed of one or more tasks. 
...
wrappers are commonly used to execute clustered jobs, but they add an overhead denoted \rev{as } the 
...
we consider sequential executions only. The overall runtime for the workflow in Fig.~\ref{fig:model-hc}~(left) 
...
\begin{eqnarray}
	\displaystyle\cfrac{1}{\theta_{\gamma}}\sim\Gamma\left(a_{\gamma}+n,\frac{1}{b_{\gamma}+\sum\nolimits_{i=1}^n{x_i^{\phi_{\gamma}}}}\right) \\
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.45\linewidth]{ligo_shape.pdf} \\
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.35\linewidth]{montage_shape.pdf} \\
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.5\linewidth]{cybershake_shape.pdf} \\
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.75\linewidth]{genome_shape.pdf} \\
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.5\linewidth]{sipht_shape.pdf} \\
