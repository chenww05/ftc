...
\rev{There have been a considerable amount of work trying to solve workflow-mapping problem 
using DAG scheduling heuristics~\cite{Topcuoglu2002, Blythe2005, Braun2001, Rubing2005, 
Wieczorek2005, Kalayci2010}.
In particular, HTCondor~\cite{Kalayci2010} has used matchmaking to avoid scheduling tasks to 
compute nodes without sufficient resources (CPU power, etc.). In our previous work~\cite{Chen2012}, 
we adopted a similar approach to avoid schedule workflow tasks to compute nodes with a high 
failure rate. In this work, we focus on the performance gain of task clustering and in particular, 
...
task each ($t_3$ and $t_4$). Since the clustering size is \rev{minimum ($k = 1$)}, VR 
performs no vertical clustering anymore and will continue retrying $t_3$ and $t_4$ (but 
...
\begin{figure*}[!htb]
	...
	\begin{subfigure}{.32\linewidth}
		\centering
		\includegraphics[width=.7\linewidth]{ligo_shape.pdf}
		\caption{LIGO}
		\label{fig:evaluation-shape-ligo}
	\end{subfigure}
	\begin{subfigure}{.32\linewidth}
		\centering
		\includegraphics[width=.5\linewidth]{montage_shape.pdf}
		\caption{Montage}
		\label{fig:evaluation-shape-montage}
	\end{subfigure}
	\begin{subfigure}{.32\linewidth}
		\centering
		\includegraphics[width=.7\linewidth]{cybershake_shape.pdf}
		\caption{CyberShake}
	...
	\begin{subfigure}{.32\linewidth}
		\centering
		\includegraphics[width=.7\linewidth]{sipht_shape.pdf}
		\caption{SIPHT}
...
\rev{thereby } generating more task failures, the performance improvement of our fault-tolerant 
