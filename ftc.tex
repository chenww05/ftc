...
approaches yield makespan improvements in such scenario. In addition, we propose a dynamic 
task clustering strategy \rev{to optimize } the workflow's makespan by dynamically adjusting the 
clustering granularity when failures arise. A trace-based simulation of 5 real workflows shows 
that our dynamic method is able to adapt from unexpected behaviors, and yields better makespan 
...
makespan \rev{(time between the start and finish of all workflow tasks)}; and 3) the performance 
...
compute nodes without sufficient resources (CPU power, etc.). In a previous work~\cite{Chen2012}, 
we adopted a similar approach to avoid schedule workflow tasks to compute nodes with a high 
failure rate. In this work, we focus on the performance gain of task clustering, in particular on
how to adjust the clustering size to balance the cost of task retry and the scheduling overheads.}
...
\vspace{4pt}
\noindent \textbf{Job Scheduler/Local Queue.} Manage individual workflow jobs and 
...
In addition, system overheads also include data transfer delay caused by staging-in and 
...
is \rev{$runtime_l= s_1+t_1+\max(s_2,s_3)+\max(t_2,t_3)+s_4+t_4$}, while the overall runtime for the 
...
bandwidth is 15MB according to the execution environment in FutureGrid from where our 
...
\rev{Our experimental results show that adaptive task re-clustering methods outcomes 
