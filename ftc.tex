...
\begin{algorithm}[!htb]
	...
	\begin{algorithmic}[1]
		...
		\Procedure{Reclustering}{$J$}\Comment{$J$ is a failed job}
			...
			\State $W \gets W + J_{new}$ \Comment{and reexecute it}
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[!htb]
	...
	\begin{algorithmic}[1]
		...
		\Procedure{Clustering}{$W,C$}
			\For{$level < depth(W)$}
				\State $TL\gets $\ \Call{GetTasksAtLevel}{$W,level$} \Comment{Partition $W$ based on depth}
				\State $CL, TL_{merged}\gets$  \ \Call{Merge}{$TL,C$} \Comment{Returns a list of clustered jobs}
		...
		\Procedure{Merge}{$TL, C$}
			...
			\State $CL\gets$\{\}\Comment{An empty list of clustered jobs}
			\ForAll{$t$ in $TL$}
				\State $J\gets$ \{$t$\}\Comment{An empty job}
