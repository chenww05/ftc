...
\begin{abstract}
%\boldmath
\rev{Task clustering has been proven to be an effective method to reduce execution overhead 
...
fault-tolerant static clustering strategies to improve the runtime performance of \rev{workflow 
...
\rev{solutions } yield makespan improvements in such scenarios. In addition, we propose a dynamic 
task clustering strategy \rev{to optimize } the workflow's makespan by dynamically adjusting the 
clustering granularity when failures arise. A trace-based simulation of \rev{five } real workflows shows 
that our dynamic method is able to adapt \rev{to } unexpected behaviors, and yields better makespan 
...
Task clustering~\cite{Chen2013a, Singh2008, Chen2012, Ferreira-granularity-2013, Maheshwari2012, 
...
\rev{is the most common technique } used to address execution overheads and increase the 
computational granularity of workflow tasks \rev{executed } on distributed 
resources~\cite{Singh2008, Chen2013a, Chen2012, Ferreira-granularity-2013}.
However, existing clustering strategies ignore or underestimate the impact of failures on the system, 
despite \rev{their significant effect on } large-scale distributed 
...
workflows. The most common technique is to \rev{retry } the failed job~\cite{5071878, 5671856, 4534303}. 
...
\rev{recomputed tasks will succeed}. As an alternative, jobs can be replicated to avoid failures 
...
executions can be periodically checkpointed to limit the \rev{amount of retried work}. However, 
...
on two real scientific workflows. In this work, we \rev{extend our } previous work by studying 
...
makespan \rev{(turnaround time to execute all workflow tasks)}; and 3) the performance 
...
Failure analysis and modeling \rev{of } computer systems have been widely studied in 
...
been addressed. For instance, the Pegasus WMS~\cite{Deelman2005, Deelman-FGCS-2014} has incorporated 
...
\rev{Several works have addressed the workflow-mapping problem by
...
fits several WMS such as Pegasus~\cite{Deelman2005, Deelman-FGCS-2014}, 
...
defined in the executable workflow to perform computations. \rev{The time span between the 
...
supervise their execution on local and remote resources. The \rev{time span between the
job submission to the scheduler and the beginning of its execution } in a worker node is 
denoted as the \textbf{queue delay}. \rev{This delay } reflects both the efficiency of the 
scheduler and the resource availability. 

\vspace{4pt}
\noindent \textbf{Job Wrapper.} Extracts tasks from clustered jobs and executes them \rev{on } 
...
tasks are poorly clustered~\cite{Chen2011}, \rev{in particular for tasks with very short runtimes. }
Fig.~\ref{fig:model-odag} \rev{illustrates } how we augment a DAG to be an o-DAG with the
capability to represent system overheads ($s$) such as the workflow engine and queue delays. 
In addition, system overheads also include data transfer \rev{delays } caused by staging-in and 
staging-out data. This classification of system overheads is based on our prior study on 
workflow analysis~\cite{Chen2011}. \rev{Table~\ref{tab:model-overheads} summarizes the 
...
wrappers are often used to execute clustered jobs, but they add an overhead \rev{defined as } the 
...
\rev{In a parallel environment}, the overall runtime for the workflow in Fig.~\ref{fig:model-hc}~(left) 
is \rev{$runtime_l= s_1+t_1+\max(s_2 + t_2, s_3 + t_3) +s_4+t_4$}, while the overall runtime for the 
clustered workflow in Fig.~\ref{fig:model-hc}~(right) is $runtime_r=s_1+t_1+s_2+c_1+t_2+t_3+s_4+t_4$.  
$runtime_l > runtime_r$ as long as \rev{$c_1 < \max(s_2,s_3)$}, which is often the case in many distributed 
...
\rev{In } situations where the scheduling and queue overheads are important, the use of task clustering 
...
have verified that the inter-arrival time of task failures \rev{better fits a Weibull distribution (defined by a shape 
...
affects the shape of a distribution\rev{, for example, whether it is symmetrical or not. }The scale parameter affects the stretching or shrinking of a 
distribution\rev{, for example, whether it is approximately uniform or it has a peak. }Both parameters control the characteristics of a distribution. For example, the mean of a 
...
$p(\theta|a, b)$ is the prior, which we \rev{have }already known from previous works; and $p(D|\theta)$ is 
...
\rev{Equation~\ref{eq:theta-1} means the posterior estimation of $\theta_{\gamma}$ is first controlled by the prior knowledge ($a_{\gamma}$ and $b_{\gamma}$) and then gradually adjusted by observed data ($n$ and $x_i$). }
...
We have assumed \rev{that }the task runtime, system overheads, and inter-arrival time between failures 
...
re-executed until it is eventually successfully \rev{completed }(since failures are assumed transient). 
...
It is difficult to find \rev{an }analytical solution of $k^*$. However, there are a few constraints that can 
...
a medium task failure rate ($\theta_{\gamma}=30$s) and a high task failure rate ($\theta_{\gamma}=20$s) \rev{respectively}. 
...
\rev{(\emph{k}) } of the jobs to reduce the impact of task failures on the workflow 
...
workflow management system (WMS)~\cite{Deelman2005, Deelman-FGCS-2014}.
...
\emph{clusters.num} to be the same as the \rev{amount } of available resources. 
In~\cite{Chen2013a,Chen2013b}, we have evaluated the runtime performance for different clustering 
granularities. Algorithm~\ref{alg:evaluation-hc} shows the pseudocode for HC. The \texttt{Clustering} 
and \texttt{Merge} procedures are \rev{invoked } in the initial task clustering process, while the 
\texttt{Reclustering} procedure is invoked when a failed job is detected by the monitoring system. 
Fig.~\ref{fig:clustering-hc} shows an example \rev{for $k$ = 4}. As a result,  there are four tasks in 
...
The selective re-clustering technique, on the other hand, \rev{merges only failed tasks within
...
to adjust the clustering size \rev{$k$}, although the clustering size \rev{may } become smaller 
after each attempt since \rev{subsequent clustered jobs may have fewer tasks}. In the example,  
\rev{$k$ } has decreased from 4 to 3. However, the optimal clustering size may not be 3, which limits 
...
uses a self-adjusted approach to reduce \rev{$k$ } to the number of failed tasks. 
...
an example where \rev{$k$ is initially set to 4}. In the first attempt, 3 tasks within the 
...
job $j_2$. In the second attempt, $j_2$ fails and then it is divided into two \rev{single task jobs }
($t_3$ and $t_4$). Since the clustering size is \rev{minimum ($k = 1$)}, VR 
...
proper order using Pegasus~\cite{Deelman2005, Deelman-FGCS-2014}. These involve the prediction of $\rho$-independent 
...
\rev{This assumption is also consistent with the setting of many real execution 
...
traces were collected. \rev{In our execution model, the network bandwidth is the maximum 
...
\vspace{5pt}
\noindent \textbf{\rev{Workload Dataset}.}
We collected workflow execution traces~\cite{Juve2013, Chen2011} (including overhead 
and task runtime information) from real runs of the \rev{5 scientific workflow applications 
previously described}. The traces are used to feed the Workflow Generator~\cite{rafael2014community} 
toolkit to \rev{create } synthetic workflows. The toolkit uses \rev{statistical data } gathered 
from \rev{traces of } actual scientific workflow executions to generate \rev{realistic}, synthetic 
workflows \rev{that resemble the real applications}. The number of inputs to be processed, 
the number of tasks in the workflow, and their composition determine the structure of the 
generated workflow. \rev{Traces, profile data, and characterizations are freely available 
...
\vspace{5pt}
\noindent \textbf{\rev{Experiment Sets}.}
...
method (HC), which \rev{is devoided } of fault-tolerant mechanisms. The goal of the 
...
\rev{scale parameter of the distribution of the } inter-arrival time of task failures). 
$\theta_{\gamma}$ ranges from 1 to 10 times of the average task runtime such that the 
workflows do not run continuously and the performance difference is visually explicit. 
...
based on the results of Experiment~1. For this experiment, we \rev{define } $\theta_{\gamma}=100$ since it 
...
In the static estimation process, only the prior knowledge \rev{(shape and scale 
...
the MLEs of the unknown parameters \rev{(scales parameters of the inter-arrival 
...
\rev{In this experiment, the prior knowledge such as the shape parameter $a_{\gamma}$, 
...
\vspace{5pt}
\noindent \textbf{\rev{Inter-arrival Time of Failures Function ($\theta_\gamma(t)$)}.}
...
to HC in a large scale. By decreasing the inter-arrival time \rev{of failures}~($\theta_{\gamma}$) and 
\rev{thereby } generating more task failures, the performance improvement of our methods 
becomes more significant. Among the three methods, DR and VR 
perform consistently better than SR, which fails to improve the \rev{overall } makespan when $\theta_{\gamma}$ 
...
does not generate long jobs and thus the performance of VR is less sensitive to \rev{the variation of the
...
\rev{Table~\ref{tab:tasks} shows the average number of tasks for the minimum 
...
\begin{table*}[!htb]
	...
	\end{tabular}
	\caption{\rev{(\emph{Experiment 1}): Average number of tasks for min and max values of $\theta_{\gamma}$ for each algorithm.}}
	\label{tab:tasks}
	\vspace{-15pt}
\end{table*}
...
adjust the clustering size to the actual optimal clustering size \rev{$k$}, which may be larger or smaller
...
methods when the system overhead is varied by using a multiplier factor $\theta_s$. 
...
\rev{gained } by the fault-tolerant methods is less significant than the performance improvements shown
...
with a pulse function of $\theta_{\gamma}$. For this experiment, we \rev{define the width of 
the pulse } $\tau = 0.1 \cdot T_c$, $0.3 \cdot T_c$, $0.5 \cdot T_c$, \rev{where $T_c$ is the period 
of the wave } (Equation~\ref{eq:pulse_function}). For $\tau = 0.1 \cdot T_c$ (Fig.~\ref{fig:exp3}.a), 
the dynamic estimation improves the makespan by up to 25.7\% \rev{when } compared to the static estimation case. For $\tau=0.3 \cdot T_c$ (Fig.~\ref{fig:exp3}.b), 
...
difference is \rev{negligible } since the inter-arrival time is close to the workflow makespan. For 
...
\rev{Our experimental results show that adaptive task re-clustering methods results in 
...
In this work, we \rev{modeled } transient failures in a distributed environment and assess their influence 
on task clustering. We \rev{proposed } three dynamic clustering methods to improve the fault tolerance 
of task clustering and \rev{applied } them to five widely used scientific workflows. Experimental results 
\rev{showed } that the proposed methods significantly improve the workflow's makespan when compared 
to an existing task clustering method used in workflow management systems. In particular, the 
Dynamic Reclustering method \rev{performed } best among all methods since it \rev{could } adjust the clustering 
size based on the Maximum Likelihood Estimation of task runtime, system overheads, and the 
inter-arrival time of failures. The Vertical Reclustering method significantly \rev{improved } the performance 
for  workflows that \rev{had } short task runtimes. The dynamic estimation process, which \rev{used } data 
collected during the workflow execution, \rev{could } further improve the overall runtime in a dynamic 
environment where the inter-arrival time of failures \rev{was } fluctuant.
...
We \rev{assumed } that the inter-arrival time of transient failures is a function of task type, which is one of 
