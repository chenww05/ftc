...
\begin{abstract}
%\boldmath
Task clustering has been proven to be an effective method to reduce execution overhead 
and improve the computational granularity of scientific workflow tasks executing on distributed 
resources. However, a job composed of multiple tasks may have a higher risk of suffering from 
failures than a single task job. Therefore, we conduct a theoretical analysis on the impact of 
...
parameter estimation process to model these performance issues. We further propose 3 
...
that our dynamic method is able to adapt to unexpected behaviors, and yields better makespan 
...
tasks in which the runtime may be shorter than the system overhead---the 
period of time during which miscellaneous work other than the user'۪s computation is performed. 
Task clustering~\cite{Singh2008, Chen2012, Ferreira-granularity-2013, Maheshwari2012, 
FerreiradaSilva-CCPE-2014, Integration2012} merges several short tasks into a single job such 
that the job runtime is increased and the overall system overhead is decreased. Task clustering is the most common technique used to address execution overheads and increase the 
...
on two real scientific workflows. In this work, we extend our previous work by studying 
1) the performance gain of using fault-tolerant task clustering methods over an 
existing task clustering technique on a larger set of workflows (five widely used scientific 
applications); 2) the performance impact of the variance of the distribution of the 
task runtimes, the system overheads, and the inter-arrival time of failures on the workflow's 
makespan (turnaround time to execute all workflow tasks); and 3) the performance 
impact of dynamic and static failure estimations with variation of the inter-arrival time of failures 
on the workflow's execution.
...
Failure analysis and modeling of computer systems have been widely studied in 
...
WMS have provided such feature.
...
Several works have addressed the workflow-mapping problem by
...
we adopted a similar approach to avoid schedule workflow tasks to compute nodes with a high 
failure rate~\cite{Chen2012}. In this work, we focus on the performance gain of task clustering, in particular on
...
within an execution site, execute jobs individually. The main components are:
...
composition system. It restructures the workflow for performance optimization, and adds 
...
job submission to the scheduler and the beginning of its execution in a worker node is 
...
tasks are poorly clustered~\cite{Chen2011}, in particular for tasks with very short runtimes. Fig.~\ref{fig:model-odag} illustrates how we augment a DAG to be an o-DAG with the
...
staging-out data. This classification of system overheads is based on our prior study on 
...
workflows do not run \rev{endlessly }and the performance difference is visually explicit. 
...
environment where the inter-arrival time of failures was fluctuant.
...
site, which may improve the accuracy of the model. \rev{We also assumed that the network bandwidth is the maximum allowed data
transfer speed between a pair of virtual machines per file. In the future, we plan to consider other models of network bandwidth.}
...
\footnotesize
This work was supported by NFS under grant number IIS-0905032, and  SI$^2$--SSI program, award number 1148515.
