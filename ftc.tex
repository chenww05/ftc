...
\title{Dynamic and Fault-Tolerant Clustering\\for Scientific Workflows}
...
\author{Weiwei~Chen,~\IEEEmembership{Student Member,~IEEE,}
	...
	\IEEEcompsocthanksitem Corresponding author: Weiwei Chen weiweich@google.com
}% <-this % stops a space
\thanks{}}
...
% The paper headers
\markboth{IEEE Transactions on Cloud Computing, February~2015}%
...
\newcommand{\note}[1]{\color{red} \textbf{[TODO]} #1\color{black}}
...
\begin{abstract}
%\boldmath
Task clustering has proven to be an effective method to reduce execution overhead 
and \rev{to } improve the computational granularity of scientific workflow tasks executing on distributed 
resources. However, a job composed of multiple tasks may have a higher risk of suffering from 
failures than a single task job. \rev{In this paper}, we conduct a theoretical analysis \rev{of } the impact of 
transient failures on the runtime performance of scientific workflow executions. We propose a 
general task failure modeling framework that uses a Maximum Likelihood estimation-based 
parameter estimation process to model \rev{workflow performance}. We further propose 3 
fault-tolerant clustering strategies to improve the runtime performance of workflow 
executions in faulty execution environments. Experimental results show that failures can have 
significant impact on executions where task clustering policies are not fault-tolerant, and that our 
solutions yield makespan improvements in such scenarios. In addition, we propose a dynamic 
task clustering strategy to optimize the workflow's makespan by dynamically adjusting the 
clustering granularity when failures arise. A trace-based simulation of five real workflows shows 
that our dynamic method is able to adapt to unexpected behaviors, and yields better \rev{makespans }
...
tasks, \rev{where the task } runtime may be shorter than the system overhead---the 
period of time during which miscellaneous work other than the user'۪s computation is performed. 
Task clustering \rev{methods}~\cite{Singh2008, Chen2012, Ferreira-granularity-2013, Maheshwari2012, 
FerreiradaSilva-CCPE-2014, Integration2012} \rev{merge } several short tasks into a single job such 
that the job runtime is increased and the overall system overhead is decreased. Task clustering 
is the most common technique used to address execution overheads and increase the 
computational granularity of workflow tasks executed on distributed 
resources~\cite{Singh2008, Chen2012, Ferreira-granularity-2013}.
However, existing clustering strategies ignore or underestimate the impact of failures on the system, 
despite their significant effect on large-scale distributed 
...
In this work, we \rev{focus particularly } on transient failures since they are expected to be more prevalent 
than permanent failures~\cite{Zhang2004}. 

A clustered job consists of multiple tasks. If a task within a clustered job fails (i.e., \rev{is } terminated 
...
workflows. The most common technique is to retry the failed job~\cite{5071878, 5671856, 4534303}. 
However, retrying a clustered job can be expensive since completed tasks within the job \rev{usually need to }
be recomputed, thereby resource cycles are wasted. In addition, there is no guarantee that 
recomputed tasks will succeed. As an alternative, jobs can be replicated to avoid failures 
specific to a worker node~\cite{Plankensteiner2009}. However, job replication
may also waste resources, in particular for long-running jobs. To reduce resource waste, job
executions can be periodically checkpointed to limit the amount of retried work. However, 
the overhead \rev{of } performing checkpointing can limit its benefits~\cite{Zhang2004}. 
...
and the inter-arrival time of failures. The process uses the Maximum Likelihood Estimation 
(MLE) based on \emph{prior} and \emph{posterior} knowledge to build the estimates. The prior 
knowledge about the parameters is modeled as a distribution with known parameters. The 
posterior knowledge about the task execution is also modeled as a distribution with a known 
\emph{shape} parameter and an unknown \emph{scale} parameter. The shape parameter 
affects the shape of a distribution, while the scale parameter affects the stretching or shrinking 
of a distribution. 

The two first fault-tolerant task clustering methods were introduced and evaluated in~\cite{Chen2012} 
\rev{using } two real scientific workflows. In this work, we extend our previous work by: 
1)~\rev{studying } the performance gain of using fault-tolerant task clustering methods over an 
existing task clustering technique on a larger set of workflows (five widely used scientific 
applications); 2)~\rev{evaluating } the performance impact of the variance of the distribution of the 
task runtimes, the system overheads, and the inter-arrival time of failures on the workflow's 
makespan (turnaround time to execute all workflow tasks); and 3)~\rev{characterizing the performance 
impact on the workflow's execution of dynamic and static failure estimations with different inter-arrival 
times of failures}.
...
Failure analysis and modeling of computer systems have been widely studied \rev{over }  
...
been addressed. For instance, the Pegasus WMS~\cite{Deelman-FGCS-2014} has incorporated 
a task-level monitoring system, which retries a job if a \rev{task } failure is detected. Provenance data
is also tracked and used to analyze the cause of failures~\cite{Samak2011}. A survey of
fault detection, prevention, and recovery techniques in current grid WMS
%such as Askalon~\cite{fahringer2007askalon}, Chemomentum~\cite{schuller2008chemomentum}, 
%Escogitare~\cite{laforenza2007biological}, and Triana~\cite{taylor2007triana} 
is available in~\cite{plankensteiner2009fault}. The survey provides a compilation of recovery techniques
...
WMS have provided such \rev{features}.

The low performance of fine-grained tasks is a common problem in widely distributed platforms 
where the scheduling overhead and queuing times at resources are high. Several \rev{papers } have 
...
\rev{currently } available resources.

Several \rev{papers } have addressed the workflow-mapping problem by
using DAG scheduling heuristics~\cite{Topcuoglu2002, Blythe2005, Rubing2005, 
Wieczorek2005, Kalayci2010}.
In particular, HTCondor~\cite{Kalayci2010} uses matchmaking to avoid scheduling tasks to 
compute nodes without sufficient resources (CPU power, etc.). Previously, 
we adopted a similar approach to avoid \rev{scheduling } workflow tasks to compute nodes with \rev{high 
failure rates}~\cite{Chen2012}. In this work, we focus on the performance gain of task clustering, in particular on
how to adjust the clustering size to balance the cost of task retry and of the scheduling overheads.
...
Machine learning techniques have been used to predict execution time~\cite{Rubing2009,  
...
fits several WMS such as Pegasus~\cite{Deelman-FGCS-2014}, 
Askalon~\cite{fahringer2007askalon}, and Taverna~\cite{Oinn2004}. 
...
targets a homogeneous computer cluster (e.g., a dedicated cluster or a virtual cluster on Clouds). 
The submit host prepares a workflow for execution (e.g. clustering, mapping, etc.). \rev{The jobs are executed
remotely on individual worker nodes}. The main components are:
...
workflow~\cite{Deelman-FGCS-2014} provided by the user or a workflow 
composition system. It restructures the workflow \rev{to optimize performance}, and adds 
tasks for data management and provenance information generation. In this work, the 
workflow mapper is also used to merge tasks into a single clustered job 
(i.e., \emph{task clustering}). A job then is a single execution unit in the workflow execution 
system and is composed of one or more tasks. 

\vspace{4pt}
\noindent \textbf{Workflow Engine:} Executes jobs defined \rev{in } the workflow in order of their 
...
defined in the executable workflow to perform computations. The time span between the 
job release and its submission to the Job Scheduler is denoted as the \textbf{workflow 
engine delay}.

\vspace{4pt}
\noindent \textbf{Job Scheduler and Local Queue:} Manage individual workflow jobs and 
supervise their execution on local and remote resources. The time span between the
job submission to the scheduler and the beginning of its execution \rev{on } a worker node is 
denoted as the \textbf{queue delay}. This delay reflects both the efficiency of the 
scheduler and the resource availability. 

\vspace{4pt}
\noindent \textbf{Job Wrapper:} Extracts tasks from clustered jobs and executes them on 
the worker nodes. The \textbf{clustering delay} is the elapsed time of the extraction process.
...
tasks are poorly clustered~\cite{Chen2011}, in particular for tasks with very short runtimes. 
Fig.~\ref{fig:model-odag} illustrates how we augment a DAG to be an o-DAG with the
capability to represent system overheads ($s$) such as the workflow engine and queue delays. 
In addition, system overheads also include data transfer delays caused by staging-in and 
staging-out \rev{of } data. This classification of system overheads is based on our prior  
workflow analysis~\cite{Chen2011}. Table~\ref{tab:model-overheads} summarizes the 
system overheads and task runtimes of three real scientific workflows executed on a real 
distributed platform. Details about these scientific workflow applications will be presented 
in Section~\ref{sec:applications}.

\begin{figure}[!t]
	\centering
	\includegraphics[width=0.55\linewidth]{odag.pdf}
	\captionof{figure}{Extending DAG to o-DAG ($s$ is a system overhead).}
...
\begin{table*}[!t]
	...
	\begin{tabular}{lrrrrrr}
		\hline
		 & \multicolumn{1}{c}{Number} & \multicolumn{1}{c}{Number} & \multicolumn{1}{c}{Average Workflow} & \multicolumn{1}{c}{Average} &  \multicolumn{1}{c}{Average} \\
		Workflow	& of Tasks & of Nodes	 &  Engine Delay & Queue Delay & Task Runtime \\
		\hline
		...
		\hline
	\end{tabular}
	\caption{System overheads and task runtimes gathered from scientific workflow execution traces~\cite{Chen2011}.}
	\label{tab:model-overheads}
	\vspace{-15pt}
\end{table*} 
...
Clustering} (VC) merges tasks within a pipeline of the workflow. Tasks \rev{in } the same pipeline share 
...
wrappers are often used to execute clustered jobs, but they add an overhead defined as the 
...
$t_3$ in $j_1$ can be executed in sequence or in parallel, if parallelism is supported.
In a parallel environment, the overall runtime for the workflow in Fig.~\ref{fig:model-hc}~(left) 
is $runtime_l= s_1+t_1+\max(s_2 + t_2, s_3 + t_3) +s_4+t_4$, while the overall runtime for the 
clustered workflow in Fig.~\ref{fig:model-hc}~(right) is $runtime_r=s_1+t_1+s_2+c_1+t_2+t_3+s_4+t_4$.  
$runtime_l > runtime_r$ as long as $c_1 < \max(s_2,s_3)$, which is often the case in many distributed 
systems since the clustering delay within a single execution node is usually shorter than the scheduling 
overhead across different execution nodes. 
...
In situations where the scheduling and queue overheads are important, the use of task clustering 
...
according to the failure rates, in particular, the task failure rate. Intuitively, if the task failure 
...
In the rest of this \rev{paper}, we will show how to adjust $k$ based on the estimated parameters of the task 
...
In our prior work~\cite{Chen2011}, we have verified that system overheads \rev{$\bm s$ }better fits a Gamma or 
a Weibull distribution rather than an Exponential or Normal distribution. Schroeder et al.~\cite{Schroeder2006} 
have verified that the inter-arrival time of task failures better fits a Weibull distribution (defined by a shape 
parameter of 0.78) rather than the Lognormal and Exponential distributions. In~\cite{McConnel}, transient 
...
affects the shape of a distribution, for example, whether it is symmetrical or not. The scale parameter affects the stretching or shrinking of a 
distribution, for example, whether it is approximately uniform or it has a peak. Both parameters control the characteristics of a distribution. For example, the mean of a 
...
function~\cite{diaconis1979conjugate}. For instance, the Inverse-Gamma family is conjugate to itself 
...
$p(\theta|a, b)$ is the prior, which we have already known from previous works; and $p(D|\theta)$ is 
...
\begin{equation}
	\theta_{\gamma}\sim\Gamma^{-1}(a_{\gamma}+n,\displaystyle b_{\gamma}+\sum\nolimits_{i=1}^n{x_i^{\phi_{\gamma}}})
	\label{eq:theta-1}
 \end{equation}

Equation~\ref{eq:theta-1} means the posterior estimation of $\theta_{\gamma}$ is \rev{initially }controlled by the prior knowledge ($a_{\gamma}$ and $b_{\gamma}$) and then gradually adjusted by observed data ($n$ and $x_i$). The MLE (Maximum Likelihood Estimation) of the scale parameter $\theta_{\gamma}$ is defined as:

\begin{equation}
	MLE(\theta_{\gamma})=\displaystyle\tfrac{b_{\gamma}+\displaystyle\sum\nolimits_{i=1}^n{x_i^{\phi_{\gamma}}}}{a_{\gamma}+n+1}
	\label{eq:mle_theta-1}
\end{equation}
The understanding of the MLE is two fold: in the initial case there is no \rev{observed }data, thus the MLE is determined
by the prior knowledge, i.e. $\displaystyle\tfrac{b_{\gamma}}{a_{\gamma}+1}$; when $n~\to~\infty$, the 
MLE~$\displaystyle\tfrac{\displaystyle\sum\nolimits_{i=1}^n{x_i^{\phi_{\gamma}}}}{n+1}\to\overline{x^{\phi_{\gamma}}}$, 
...
$b_t$ the rate parameter (or $\tfrac{1}{b_t}$ is the scale parameter), the posterior 
follows $\Gamma(a_t+n\phi_t, b_t+\sum\nolimits_{i=1}^n{t_i})$ with $a_t+n\phi_t$ as the 
shape parameter and $b_t+\sum\nolimits_{i=1}^n{t_i}$ as the rate parameter. The MLE of 
$\theta_t$ is then defined as follows: 

\begin{equation}
	MLE(\theta_t) = \displaystyle\tfrac{b_t+\displaystyle\sum\nolimits_{i=1}^n{t_i}}{a_t+n\phi_t-1}
	\label{eq:mle_theta_t}
\end{equation}
...
\begin{equation}
	MLE(\theta_s) = \frac{b_s+\sum\nolimits_{i=1}^n{s_i}}{a_s+n\phi_s-1}
	\label{eq:mle_theta_s}
\end{equation}

We have assumed that the task runtime, system overheads, and inter-arrival time between failures 
are a function of task types. The reason is that tasks at different levels (the deepest depth from 
the entry task to this task) are often of different types in scientific workflows. 
Given $n$ independent tasks at the same workflow level and the distribution of the 
...
re-executed until it is eventually successfully completed (since failures are assumed transient). 
For a given job runtime $d_i$, the retry time $N_i$ of a clustered job $i$ is defined as follows:
\begin{eqnarray}
	\displaystyle
	N_i=\frac{1}{1-F(d_i)}=\frac{1}{e^{-\left(\tfrac{d_i}{\theta_{\gamma}}\right)^{\phi_{\gamma}}}}=e^{\left(\tfrac{d_i}	{\theta_{\gamma}}\right)^{\phi_{\gamma}}} 
	\label{eq:N}
\end{eqnarray}
...
\begin{eqnarray}
	\displaystyle
	M_i=d_i \times N_i=d_i \times e^{\left(\tfrac{d_i}{\theta_{\gamma}}\right)^{\phi_{\gamma}}} 
	\label{eq:M}
\end{eqnarray}
...
\begin{eqnarray}
	\tfrac{1}{\theta_{\gamma}}\sim\Gamma\left(a_{\gamma}+n,\tfrac{1}{b_{\gamma}+\sum\nolimits_{i=1}^n{x_i^{\phi_{\gamma}}}}\right) \\
	MLE\left(\tfrac{1}{\theta_{\gamma}}\right)=\cfrac{a_{\gamma}+n-1}{b_{\gamma}+\sum\nolimits_{i=1}^n{x_i^{\phi_{\gamma}}}}
	\label{eq:mle_1_theta}
 \end{eqnarray}

\noindent $M_i$ is a monotonic increasing function of both $d_i$ and $\tfrac{1}{\theta_{\gamma}}$, 
and the two random variables are independent of each other, therefore:
\begin{equation} 
	MLE(M_i)=MLE(d_i)e^{\left(\displaystyle MLE(d_i)MLE\left(\tfrac{1}{\theta_{\gamma}}\right)\right)^{\phi_{\gamma}}} 
	\label{eq:MLE-M-i}
\end{equation}
...
$\tfrac{1}{\theta_{\gamma}}$), $M_i$ is a Gamma distribution, and each $M_i$ 
has the same distribution parameters, therefore:
\begin{eqnarray} 
	\label{eq:MLE-M}
	\bm{M}&=&\cfrac{1}{r} \times \sum\nolimits_{i=1}^{\tfrac{n}{k}}M_i\sim\Gamma \nonumber\\
	MLE(\bm{M})&=&\tfrac{n}{rk} \times MLE(M_i)\nonumber\\
	&=&\tfrac{n}{rk} \times MLE(d_i)e^{\left(MLE(d_i)MLE\left(\tfrac{1}{\theta_{\gamma}}\right)\right)^{\phi_{\gamma}}} 
\end{eqnarray}
where $r$ is the number of resources. In this work, we consider a compute cluster as a homogeneous cluster, which is often the case 
...
stands for the argument ($k$) of the minimum~\cite{argmax}, 
...
\begin{figure}[!b]
	\centering
	\vspace{-10pt}
	\includegraphics[width=0.7\linewidth]{model_makespan2.eps}
...
\begin{figure}[!b]
	\centering
	\includegraphics[width=0.7\linewidth]{model_size2.eps}
	\caption{Optimal clustering size ($k^*$) with different  $\theta_{\gamma}$ ($n=1000$, $r=20$, $\phi_t=5$s, $\phi_s=50$s).}
	\label{fig:model-size}
\end{figure}

It is \rev{not trivial }to find an analytical solution of $k^*$. However, there are a few constraints that can 
...
a medium task failure rate ($\theta_{\gamma}=30$s) and a high task failure rate ($\theta_{\gamma}=20$s) respectively. 
...
the \texttt{mProjectPP} tasks of the Montage workflow (Section \ref{sec:experiments}).
...
Inappropriate task clustering may negatively impact the workflow makespan 
in faulty distributed environments. In this section, we propose three fault-tolerant 
task clustering methods---Selective Reclustering (\emph{SR}), Dynamic Reclustering 
(\emph{DR}), and Vertical Reclustering (\emph{VR})---that adjust the clustering size 
(\emph{k}) of the jobs to reduce the impact of task failures on the workflow 
execution. These methods are based on the Horizontal Clustering (HC)~\cite{Singh2008} 
technique that has been implemented and used in the Pegasus 
workflow management system (WMS)~\cite{Deelman-FGCS-2014}.
...
clustered jobs per horizontal level of the workflow (\emph{clusters.num}). For simplicity, we set 
\emph{clusters.num} to be the same as the amount of available resources. 
In~\cite{Chen2013b}, we have evaluated the runtime performance for different clustering 
granularities. Algorithm~\ref{alg:evaluation-hc} shows the pseudocode for HC. The \texttt{Clustering} 
and \texttt{Merge} procedures are invoked in the initial task clustering process, while the 
\texttt{Reclustering} procedure is invoked when a failed job is detected by the monitoring system. 
Fig.~\ref{fig:clustering-hc} shows an example for $k$ = 4. As a result,  there are four tasks in 
a clustered job. During execution, three out of these tasks ($t_1$, $t_2$, $t_3$) fail. Due to the lack 
of an adaptive mechanism, HC keeps retrying all of the four tasks in the following attempts until all 
of them succeed.
...
\begin{algorithm}[!t]
	...
	\begin{algorithmic}[1]
		...
		\Procedure{Clustering}{$W,C$}
			\For{$level < depth(W)$}
				\State $TL\gets $\ \Call{TasksAtLevel}{$W,level$} \Comment{Divide $W$ based on depth}
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.9\linewidth]{hcr.pdf}
...
The selective re-clustering technique, on the other hand, merges only failed tasks within
a clustered job into a new clustered job. Algorithm~\ref{alg:evaluation-sr}
...
to adjust the clustering size $k$, although the clustering size may become smaller 
after each attempt since subsequent clustered jobs may have fewer tasks. In the example,  
$k$ has decreased from 4 to 3. However, the optimal clustering size may not be 3, which limits 
...
incorporated into existing WMS with minimum impact on the workflow execution efficiency 
...
uses a self-adjusted approach to reduce $k$ to the number of failed tasks. 
...
an example where $k$ is initially set to 4. In the first attempt, 3 tasks within the 
clustered job have failed. Therefore, there are only 3 tasks to be retried, and thus the 
clustering size should be decreased to 2 accordingly. Two new clustered jobs $j_2$ 
(containing $t_1$ and $t_2$) and $j_3$ (containing $t_3$) are created. Reducing the 
granularity of failed clustered jobs may decrease the probability of future failures, since 
at least one of the jobs will execute on a different worker node.
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.82\linewidth]{sr.pdf}
...
\end{algorithm}
 ...
 \begin{figure}[!b]
	\centering
	\includegraphics[width=0.77\linewidth]{dr.pdf}
	\caption{An example of Dynamic Reclustering (red boxes are failed tasks). The clustering size $k$ is adjusted to 2 and thus failed tasks are merged into two new clustered jobs.}
...
\begin{algorithm}[!t]
	...
	\begin{algorithmic}[1]
		...
		\Procedure{Clustering}{$W$}
			\For{$level < depth(W)$}
				\State $TL\gets $\ \Call{TasksAtLevel}{$W,level$} \Comment{Divide $W$ based on depth}
				\State $CL, TL_{merged}\gets$  \ \Call{Merge}{$TL$} \Comment{List of clustered jobs}
		...
		\Procedure{Merge}{$TL$}
			...
			\ForAll{$t$ in $TL$}
				\State $J\gets$ \{$t$\}
				\While{$t$ has only 1 child $t_{child}$ and $t_{child}$ has only 1 parent}
		...
		\Procedure{Reclustering}{$J$}\Comment{$J$ is a failed job}
			...
			\ForAll{Task $t$ in $TL$}
				...
				\EndIf
				\If{$J_{new}.size() > k^*$}
					\State $W \gets W + J_{new}$; $J_{new}\gets$\{\}
				\EndIf
			\EndFor
			\State $W \gets W + J_{new}$ \Comment{Re-execute it}
		\EndProcedure
...
job $j_2$. In the second attempt, $j_2$ fails and then it is divided into two single task jobs
($t_3$ and $t_4$). Since the clustering size is minimum ($k = 1$), VR 
performs no vertical clustering and continue retrying $t_3$ and $t_4$ (but 
still following their data dependency) until they succeed. Algorithm \ref{alg:evaluation-vr} 
shows the pseudocode for the VR method. 
...
\begin{figure*}[!htb]
	...
	\begin{subfigure}{.32\linewidth}
		\centering
		\includegraphics[width=.65\linewidth]{ligo_shape.pdf}
		\caption{LIGO}
		\label{fig:evaluation-shape-ligo}
	\end{subfigure}
	\begin{subfigure}{.32\linewidth}
		\centering
		\includegraphics[width=.45\linewidth]{montage_shape.pdf}
		\caption{Montage}
		\label{fig:evaluation-shape-montage}
	\end{subfigure}
	\begin{subfigure}{.32\linewidth}
		\centering
		\includegraphics[width=.65\linewidth]{cybershake_shape.pdf}
		\caption{CyberShake}
		\label{fig:evaluation-shape-cybershake}
	\end{subfigure}
	\begin{subfigure}{.32\linewidth}
		\centering
		\includegraphics[width=.95\linewidth]{genome_shape.pdf}
		\caption{Epigenomics (multiple branches)}
		\label{fig:evaluation-shape-genome}
	\end{subfigure}
	\begin{subfigure}{.32\linewidth}
		\centering
		\includegraphics[width=.65\linewidth]{sipht_shape.pdf}
		\caption{SIPHT}
		\label{fig:evaluation-shape-sipht}
	\end{subfigure}

	\caption{Simplified visualization of the scientific workflows.}
	\label{fig:evaluation-shape}
	\vspace{-15pt}
\end{figure*}
...
proper order using Pegasus~\cite{Deelman-FGCS-2014}. These involve the prediction of $\rho$-independent 
...
\begin{table}[!htb]
	...
	\begin{tabular}{lrrrr}
		\hline
		 & \multicolumn{1}{c}{\# Tasks} & \multicolumn{1}{c}{Avg. Data Size} &  \multicolumn{1}{c}{Avg. Task Runtime} \\
		\hline
		LIGO 		&800		& 5 MB	& 228s\\
		...
		\hline
	\end{tabular}
	\caption{Summary of the main workflow characteristics.}
...
simulator with the fault-tolerant clustering methods to simulate a controlled distributed 
environment, where system (i.e., the inter-arrival time of failures) and workflow (i.e., avg. 
task runtime) settings can be varied to fully explore the performance of our fault-tolerant 
clustering algorithms. WorkflowSim is an open source workflow simulator that extends 
CloudSim~\cite{Calheiros2011} by providing support for task clustering, task scheduling, 
and resource provisioning at the workflow level. It has been recently used in multiple 
workflow study areas~\cite{WorkflowSim,Chen2012, jrad2013broker} and its correctness 
has been verified in~\cite{WorkflowSim}.

The simulated computing platform is composed of 20 single homogeneous core virtual 
machines (worker nodes), which is the quota per user of some typical distributed 
environments such as Amazon EC2~\cite{AmazonAWS} and FutureGrid~\cite{FutureGrid}. 
This assumption is also consistent with the setting of many real execution 
experiments~\cite{Juve2013}. Each simulated virtual machine (VM) has 512MB of memory 
and the capacity to process 1,000 million instructions per second. The default network 
bandwidth is 15MB according to the execution environment in FutureGrid, where our 
traces were collected. In our execution model, the network bandwidth is the maximum 
allowed data transfer speed between a pair of virtual machines per file. By default, tasks at 
the same horizontal level are merged into 20 clustered jobs, which is a simple granularity 
control selection based on the strength of task clustering (as shown in~\cite{Chen2013b}).


\vspace{5pt}
\noindent \textbf{Workload Dataset.}
We collected workflow execution traces~\cite{Juve2013, Chen2011} (including overhead 
and task runtime information) from real runs of the 5 scientific workflow applications 
previously described. The traces are used to feed the Workflow Generator~\cite{rafael2014community} 
toolkit to create synthetic workflows. The toolkit uses statistical data gathered 
from traces of actual scientific workflow executions to generate realistic, synthetic 
workflows that resemble the real applications. The number of inputs to be processed, 
the number of tasks in the workflow, and their composition determine the structure of the 
generated workflow. Traces, profile data, and characterizations are freely available 
online for the community~\cite{WorkflowArchive}. For the experiments a synthetic workflow
was generated for each workflow application according to the characteristics shown in 
Table~\ref{tab:evaluation-workflows}.


\vspace{5pt}
\noindent \textbf{Experiment Sets.}
Three sets of experiments are conducted. Experiment 1 evaluates the performance of 
our fault-tolerant clustering methods (DR, VR, and SR) over an existing task clustering 
method (HC), which is devoided of fault-tolerant mechanisms. The goal of the 
experiment is to identify conditions where each method works best and worst. We also 
evaluate the performance improvement for different $\theta_{\gamma}$ values (the 
scale parameter of the distribution of the inter-arrival time of task failures). 
$\theta_{\gamma}$ ranges from 1 to 10 times of the average task runtime such that the 
workflows \rev{run in a reasonable amount of time } and the performance difference is visually explicit. 

Experiment 2 evaluates the performance impact of the variation of the average task runtime per level 
(defined as the average \rev{task runtime }of all the tasks per level), and the average system overheads per level for one 
scientific workflow application (CyberShake). In particular, we are interested in the performance of DR 
based on the results of Experiment~1. For this experiment, we define $\theta_{\gamma}=100$ since it 
...
Experiment 3 evaluates the performance of the static and dynamic estimation. 
In the static estimation process, only the prior knowledge (shape and scale 
parameters of a Inverse-Gamma distribution for the inter-arrival time of failures, and a 
Gamma distribution for the task runtime and system overhead) is used to estimate 
the MLEs of the unknown parameters (scales parameters of the inter-arrival 
failures $\theta_\gamma$, the task runtime $\theta_t$, and the system overhead 
$\theta_s$). In contrast, the dynamic estimation process leverages the runtime 
data collected during the execution and update the MLEs respectively.
In this experiment, the prior knowledge such as the shape parameter $a_{\gamma}$, 
and the scale parameter $b_{\gamma}$ of the inter-arrival time of failures $\theta_{\gamma}$ 
are set manually based on our experience and other researchers' work~\cite{Schroeder2006}. 
The runtime data, such as the series of the actual inter-arrival time of tasks $x_i$, 
are collected during the simulation execution and used to adjust all the MLEs 
based on 
Equations~\ref{eq:mle_theta-1},~\ref{eq:mle_theta_t},~\ref{eq:mle_theta_s},~\ref{eq:mle_theta_d},~\ref{eq:mle_1_theta}.


\vspace{5pt}
\noindent \textbf{Inter-arrival Time of Failures Function ($\theta_\gamma(t)$).}
The experiments use two sets of the $\theta_{\gamma}$ function. The first is a step 
function (Fig.~\ref{fig:evaluation_step_signal}), in which $\theta_{\gamma}$ is 
decreased from 500 seconds to 50 seconds at time $T_d$. The step function of 
$\theta_{\gamma}$ at time $t_c$ is defined as follows:
...
the estimation of the workflow makespan. Theoretically, the later we change $\theta_{\gamma}$, 
...
1,000 seconds to 10,000 seconds based on the estimation of the workflow's makespan, and 
...
to HC in a large scale. By decreasing the inter-arrival time of failures~($\theta_{\gamma}$) and 
thereby generating more task failures, the performance improvement of our methods 
becomes more significant. Among the three methods, DR and VR 
perform consistently better than SR, which fails to improve the overall makespan when $\theta_{\gamma}$ 
...
does not generate long jobs and thus the performance of VR is less sensitive to the variation of the
scale parameter of the distribution of the inter-arrival time of failures $\theta_{\gamma}$. 

\begin{figure}[!b]
	\centering
	\includegraphics[width=.95\linewidth]{t.eps}
...
\begin{figure}[!b]
	\centering
	\includegraphics[width=.95\linewidth]{d.eps}
	\caption{(\emph{Experiment 2}): Influence of varying system overhead ($\theta_s$) on makespan for the CyberShake workflow.}
	\label{fig:expr_d}
\end{figure}

Table~\ref{tab:tasks} shows the average number of tasks for the minimum 
(min($\theta_{\gamma}$)) and maximum (max($\theta_{\gamma}$)) values of 
the scale parameter of the inter-arrival time of failures. Most of the algorithms 
have same performance when the inter-arrival time of failures is sparse, except 
for HC. For small $\theta_{\gamma}$ values, DR significantly reduces the number 
of tasks.
...
\begin{table*}[!htb]
	...
	\begin{tabular}{l  r r | r r | r r | r r}
		\hline
		\multirow{2}{*}{Workflow} & \multicolumn{2}{c|}{HC} & \multicolumn{2}{c|}{VR} & \multicolumn{2}{c|}{SR} & \multicolumn{2}{c}{DR} \\
		& min($\theta_{\gamma}$) & max($\theta_{\gamma}$) & min($\theta_{\gamma}$) & max($\theta_{\gamma}$) & min($\theta_{\gamma}$) & max($\theta_{\gamma}$) & min($\theta_{\gamma}$) & max($\theta_{\gamma}$) \\ \hline
		...
		SIPHT		&18803	& 1693	& 1158	& 1016	& 1640	& 1015	& 1113	& 1018 \\ \hline
	\end{tabular}
	\caption{(\emph{Experiment 1}): Average number of tasks for min and max values of $\theta_{\gamma}$ for each algorithm.}
	\label{tab:tasks}
	\vspace{-15pt}
\end{table*}
...
methods,  when the task runtime is varied by a multiplier factor \rev{$f_{r}$. }By increasing the task runtime, HC has the most negative impact on the workflow's makespan 
...
adjust the clustering size to the actual optimal clustering size $k$, which may be larger or smaller
...
methods when the system overhead is varied by using a multiplier factor \rev{$f_o$. }
...
gained by the fault-tolerant methods is less significant than the performance improvements shown
...
$\theta_\gamma$ is close to the workflow makespan. Therefore, the influence of $\theta_\gamma$ is
...
\begin{figure}[!b]
	\centering
	\vspace{-15pt}
	\includegraphics[width=.95\linewidth]{versus.eps}
...
with a pulse function of $\theta_{\gamma}$. For this experiment, we define the width of 
the pulse $\tau = 0.1 \cdot T_c$, $0.3 \cdot T_c$, $0.5 \cdot T_c$, where $T_c$ is the period 
of the wave (Equation~\ref{eq:pulse_function}). For $\tau = 0.1 \cdot T_c$ (Fig.~\ref{fig:exp3}.a), 
the dynamic estimation improves the makespan by up to 25.7\% when compared to the static estimation case. For $\tau=0.3 \cdot T_c$ (Fig.~\ref{fig:exp3}.b), 
...
difference is negligible since the inter-arrival time is close to the workflow makespan. For 
$\tau = 0.5 \cdot T_c$, the gain of dynamic over static estimation is negligible
and nearly constant (up to 9.1\%) since $\theta_{\gamma}$ has equal influence on the failure occurrence
regardless of its value. 

\begin{figure}[!t]
	\centering
	\includegraphics[width=.95\linewidth]{pulse_t10.eps} \\
	\small (a) Pulse $\tau=0.1 \cdot T_c$ \\ \vspace{4pt}
	\includegraphics[width=.95\linewidth]{pulse_t30.eps} \\
	\small (b) Pulse $\tau=0.3 \cdot T_c$ \\ \vspace{4pt}
	\includegraphics[width=.95\linewidth]{pulse_t50.eps} \\
	\small (c) Pulse $\tau=0.5 \cdot T_c$
	\caption{(\emph{Experiment 3}): Performance evaluation of the static and dynamic estimations for the CyberShake workflow using a pulse function.}
	\label{fig:exp3}
	\vspace{-15pt}
\end{figure}

Our experimental results show that adaptive task re-clustering methods results in 
better performance than simple static methods that retry failed clustered jobs. However,
the performance gain may be significantly impacted if the job clustering size is not 
adjusted to the nearly optimal size. The Dynamic Reclustering algorithm outperforms
most of the methods for the 5 workflow applications, however, it yields poorer 
...
In this work, we modeled transient failures in a distributed environment and assess their influence 
on task clustering. We proposed three dynamic clustering methods to improve the fault tolerance 
of task clustering and applied them to five widely used scientific workflows. Experimental results 
showed that the proposed methods significantly improve the workflow's makespan when compared 
to an existing task clustering method used in workflow management systems. In particular, the 
Dynamic Reclustering method performed best among all methods since it could adjust the clustering 
size based on the Maximum Likelihood Estimation of task runtime, system overheads, and the 
inter-arrival time of failures. The Vertical Reclustering method significantly improved the performance 
for  workflows that had short task runtimes. The dynamic estimation process, which used data 
collected during the workflow execution, could further improve the overall runtime in a dynamic 
environment where the inter-arrival time of failures \rev{fluctuated}.

This work focused on the evaluation of fault-tolerant task clustering techniques on homogeneous 
...
We assumed that the inter-arrival time of transient failures is a function of task type, which is one of 
the major impact factors. In the future, we plan to consider other factors such as the execution 
site, which may improve the accuracy of the model. \rev{In this paper we assumed that the network 
bandwidth is the maximum possible data transfer speed between a pair of virtual machines per file. 
Future work will consider different network models to explore their impact on our fault-tolerant 
...
\footnotesize
This work was supported by NFS under grant number IIS-0905032, and  SI$^2$--SSI program, award number \rev{ACI--1148515}.
We thank Boleslaw Szymanski, Gideon Juve, 
