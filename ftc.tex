...
online for the community~\cite{WorkflowArchive}. For the experiments we generated, each workflow application has one synthetic workflow and the number of tasks in each workflow is shown in Table~\ref{tab:evaluation-workflows}.
...
\rev{In this experiment, the prior knowledge, such as the shape parameter $a_{\gamma}$ and the scale parameter $b_{\gamma}$ of the inter-arrival time of failures $\theta_{\gamma}$ are set manually based on our experience or other researchers' work~\cite{Schroeder2006}. The runtime data, such as the series of the actual inter-arrival time of tasks $x_i$, are collected during the simulation from WorkflowSim and used to adjust all the MLEs based on Equation~\ref{eq:mle_theta-1},~\ref{eq:mle_theta_t},~\ref{eq:mle_theta_s},~\ref{eq:mle_theta_d},~\ref{eq:mle_1_theta}.}
